...
export function LoginForm({ 
  ...
  useEffect(() => {
    const subscription = form.watch((value, { name }) => {
      if (name && value[name]) {
        debouncedTrigger(name as keyof LoginFormData)
      }
    })
  ...
  const handleFormSubmit = useCallback(async (data: LoginFormData) => {
    ...
    try {
      await onSubmit(data as LoginRequest)
    } catch {
      // 登录失败时，聚焦到密码字段
  ...
  const handleOAuthLogin = useCallback(async (provider: string) => {
    ...
    // 注意：成功的OAuth会跳转到第三方页面，所以不需要重置状态
  }, [onOAuthLogin, loading, isSubmitting, oauthLoading])

  const togglePasswordVisibility = useCallback(() => {
    setShowPassword(prev => !prev)
  }, [])

  const isFormDisabled = loading || isSubmitting || oauthLoading !== null
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input
                    ...
                    aria-describedby={fieldState.error ? `email-error-${field.name}` : undefined}
                  />
                </FormControl>
                <FormMessage id={`email-error-${field.name}`} />
              </FormItem>
            )}
          />
          ...
          <FormField
            ...
            render={({ field, fieldState }) => (
              <FormItem>
                ...
                <FormControl>
                  <div className="relative">
                    <Input
                      ...
                      aria-describedby={fieldState.error ? `password-error-${field.name}` : undefined}
                    />
                    <Button
                      type="button"
                      variant="ghost"
                ...
                <FormMessage id={`password-error-${field.name}`} />
              </FormItem>
            )}
          />
